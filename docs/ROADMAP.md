...
> PRD 기반 자동 생성 | 최종 업데이트: 2026-02-20
...
| 페이즈 | 상태 | 진행률 | 설명 |
|--------|------|--------|------|
| Phase 0: 사용자 인증 | ✅ 완료 | 5/5 완료 | 이메일/Google 로그인, 세션 관리 |
| Phase 1: 데이터베이스 및 기반 설정 | ✅ 완료 | 7/7 완료 | DB 마이그레이션, RLS, RPC, 타입, 유틸리티 |
| Phase 2: 이벤트 핵심 기능 (CRUD) | ✅ 완료 | 8/8 완료 | 이벤트 생성/목록/상세/수정 |
| Phase 3: 참여자 관리 | ✅ 완료 | 6/6 완료 | 참석 응답, 참여자 목록, 상태 뱃지 |
| Phase 4: 초대 시스템 | ✅ 완료 | 7/7 완료 | 초대 링크, 비회원 참여, 랜딩 페이지 |
| Phase 5: 공지사항 | ✅ 완료 | 5/5 완료 | 공지 CRUD, 고정, 탭 통합 |
| Phase 6: UX 완성 및 폴리싱 | ✅ 완료 | 8/8 완료 | Toast, 로딩/빈 상태, 반응형, 상태 관리 |
...
- [x] **P0-05** 로그인 시 `redirect_to` 파라미터 지원
  - 상세: 초대 링크 접속 -> 로그인 -> 원래 페이지로 복귀 플로우
  - Phase 4에서 구현 완료 (미들웨어 + 로그인 폼 + OAuth 콜백)
...
- [x] **P2-01** 이벤트 Server Actions 구현 (생성/수정)
...
- [x] **P2-05** EventForm 컴포넌트 구현 (생성/수정 공용)
  - 상세: Client Component. 이벤트 생성과 수정에 공용으로 사용하는 폼. 필드: 제목(필수), 설명(선택), 날짜/시간(필수), 장소(선택), 최대 인원(선택). useActionState + Zod 서버 유효성 검사. 수정 모드에서는 기존 데이터 프리필. 제출 시 Server Action 호출
  - 관련 파일: `components/events/event-form.tsx`
  - 의존성: P1-06, P1-07 (textarea 등 UI 컴포넌트)

- [x] **P2-06** 이벤트 생성 페이지 구현
  - 상세: `/events/new` 경로. EventForm을 생성 모드로 렌더링. 생성 완료 시 `/events/[id]` 상세 페이지로 리다이렉트. 인증된 사용자만 접근 가능
  - 관련 파일: `app/events/new/page.tsx`
  - 의존성: P2-01, P2-05

- [x] **P2-07** 이벤트 상세 페이지 구현 (정보 탭)
  - 상세: `/events/[id]` 경로. EventDetailHeader(제목, 날짜, 장소, 참여자 현황) + 탭 구성(정보/참여자/공지). Phase 2에서는 정보 탭만 구현, 참여자/공지 탭은 각 Phase에서 추가. 주최자에게만 수정 버튼 표시. shadcn/ui Tabs 컴포넌트 활용. Suspense 패턴 적용
  - 관련 파일: `app/events/[id]/page.tsx`, `components/events/event-detail-header.tsx`
  - 의존성: P2-01, P1-07 (tabs 컴포넌트)

- [x] **P2-08** 이벤트 수정 페이지 구현
  - 상세: `/events/[id]/edit` 경로. 기존 이벤트 데이터를 조회하여 EventForm에 프리필. 주최자만 접근 가능 (서버에서 권한 검증). 수정 완료 시 `/events/[id]` 상세 페이지로 리다이렉트. Suspense 패턴 적용
  - 관련 파일: `app/events/[id]/edit/page.tsx`
  - 의존성: P2-01, P2-05

### 완료 기준 (Definition of Done)

- [x] 인증된 사용자가 이벤트를 생성할 수 있음 (제목, 설명, 날짜, 장소, 최대 인원)
- [x] 이벤트 생성 시 8자리 초대 코드가 자동 생성됨
- [x] 이벤트 목록에서 "내가 만든"과 "참여 중인" 이벤트가 분리 표시됨
- [x] 이벤트 상세 페이지에서 이벤트 정보를 확인할 수 있음
- [x] 주최자만 이벤트를 수정할 수 있음
- [x] 비주최자가 수정 페이지에 접근하면 차단됨
...
- [x] **P3-01** 참여자 관련 Server Actions 구현
  - 상세: `respondToEvent` - 참석/불참/미정 응답 upsert (이미 응답한 경우 상태 변경, FR-03-01 + FR-03-02). user_id + event_id 기준으로 기존 응답 확인 후 INSERT 또는 UPDATE. revalidatePath로 상세 페이지 캐시 무효화. Zod 스키마로 입력 유효성 검사
  - 관련 파일: `app/events/actions.ts` (`respondToEvent` 함수)
  - 의존성: P1-02, P1-06

- [x] **P3-02** RsvpBadge 컴포넌트 구현
...
- [x] **P3-04** ParticipantItem 컴포넌트 구현
...
- [x] **P3-05** ParticipantList 컴포넌트 구현
...
- [x] 로그인한 사용자가 이벤트에 참석/불참/미정 응답 가능
- [x] 이미 응답한 사용자가 응답을 변경할 수 있음
- [x] 참여자 목록이 상태별(참석/미정/불참)로 그룹화되어 표시됨
- [x] 이벤트 상세 페이지 "참여자" 탭에서 목록과 응답 폼이 동작함
...
- [x] **P4-05** 초대 관련 Server Actions 구현
...
- [x] 이벤트 상세 페이지에서 초대 링크를 클립보드에 복사할 수 있음
- [x] 초대 링크(`/invite/[code]`)로 접속하면 이벤트 정보가 표시됨
- [x] 비회원이 이름만 입력하여 이벤트에 참여할 수 있음
- [x] 비회원 참여 완료 후 확인 화면과 회원가입 유도 CTA가 표시됨
- [x] 로그인된 사용자가 초대 링크로 바로 이벤트에 참여할 수 있음
- [x] 비로그인 사용자가 초대 링크 접속 -> 로그인 -> 원래 초대 페이지로 복귀됨
- [x] 잘못된 초대 코드 접속 시 적절한 에러 화면이 표시됨
...
- [x] **P5-01** 공지사항 Server Actions 구현
  - 상세: `createAnnouncement` - 공지 작성 (주최자만, event_id + author_id + content). `deleteAnnouncement` - 공지 삭제 (작성자 또는 주최자). `toggleAnnouncementPin` - 공지 고정/해제 (주최자만, is_pinned 토글). 각 Action에서 권한 검증 포함. Zod 유효성 검사 (content 5000자 제한)
  - 관련 파일: `app/events/actions.ts`
  - 의존성: P1-03, P1-06

- [x] **P5-02** AnnouncementForm 컴포넌트 구현
  - 상세: Client Component. 공지사항 작성 폼. textarea로 내용 입력 + 작성 버튼. 주최자에게만 표시. 작성 완료 시 폼 리셋 + 목록 갱신 (revalidatePath). useActionState 패턴 사용
  - 관련 파일: `components/events/announcement-form.tsx`
  - 의존성: P5-01, P1-07 (textarea 컴포넌트)

- [x] **P5-03** AnnouncementCard 컴포넌트 구현
  - 상세: Client Component. 개별 공지사항 카드. 작성자명, 작성 시간(상대시간 포맷), 내용, 고정 아이콘(고정된 경우 border/bg 강조). 삭제 버튼(작성자 또는 주최자에게만 표시). 고정/해제 버튼(주최자에게만 표시). useActionState + bind 패턴 사용
  - 관련 파일: `components/events/announcement-card.tsx`
  - 의존성: P1-06

- [x] **P5-04** AnnouncementList 컴포넌트 구현
  - 상세: Server Component. 공지사항 목록 (FR-05-02). 고정된 공지를 상단에 표시, 나머지는 최신순 정렬. 공지가 없을 때 빈 상태 메시지 (Megaphone 아이콘). profiles 조인으로 작성자명 표시
  - 관련 파일: `components/events/announcement-list.tsx`
  - 의존성: P5-03

- [x] **P5-05** 이벤트 상세 공지 탭 통합
...
- [x] 주최자가 이벤트에 공지사항을 작성할 수 있음
- [x] 공지사항이 최신순으로 정렬되어 표시됨
- [x] 고정된 공지가 목록 상단에 표시됨
- [x] 작성자 또는 주최자가 공지를 삭제할 수 있음
- [x] 주최자가 공지를 고정/해제할 수 있음
- [x] 이벤트 상세 페이지 "공지" 탭에서 모든 기능이 동작함
...
- [x] **P6-01** Toast 알림 시스템 통합
  - 상세: sonner 라이브러리 기반 전역 알림 시스템. 이벤트 생성/수정/삭제 성공, 참석 응답 변경, 공지 작성/삭제, 참여자 제거, 에러 발생 등 모든 사용자 액션에 대한 피드백. RootLayout에 `<Toaster richColors closeButton />` 추가
  - 관련 파일: `app/layout.tsx`, `components/ui/sonner.tsx`, 각 Client Component (toast 호출)
  - 의존성: P1-07 (toast 컴포넌트)

- [x] **P6-02** 로딩 상태 및 빈 상태 UI 구현
  - 상세: (1) Next.js loading.tsx 파일을 활용한 페이지 로딩 스켈레톤 UI (이벤트 목록, 상세 페이지). (2) 이벤트/참여자/공지 각각의 빈 상태 안내 UI (아이콘 + 안내 메시지). 별도 EventEmptyState 컴포넌트 대신 각 목록 컴포넌트에 인라인 구현
  - 관련 파일: `app/events/loading.tsx`, `app/events/[id]/loading.tsx`, `app/events/page.tsx`, `components/events/participant-list.tsx`, `components/events/announcement-list.tsx`
  - 의존성: P2-03, P3-05, P5-04

- [x] **P6-03** 이벤트 삭제 기능 구현
  - 상세: `deleteEvent` Server Action (주최자만, FR-02-05). 삭제 확인 다이얼로그 (shadcn/ui Dialog). announcements + participants 수동 삭제 후 이벤트 삭제. 삭제 완료 후 `/events` 목록으로 리다이렉트
  - 관련 파일: `app/events/actions.ts`, `components/events/event-actions.tsx`
  - 의존성: P2-07, P1-07 (dialog 컴포넌트)

- [x] **P6-04** 이벤트 상태 변경 기능 구현
  - 상세: `updateEventStatus` Server Action (주최자만, FR-02-06). active -> cancelled/completed, cancelled -> active, completed -> active 상태 전환. EventActions 컴포넌트에 상태별 변경 버튼 표시. 진행 중이 아닌 이벤트에서는 참석 응답 차단
  - 관련 파일: `app/events/actions.ts`, `components/events/event-actions.tsx`
  - 의존성: P2-07, P3-03

- [x] **P6-05** 참여자 메모 기능 구현
  - 상세: RsvpForm에 메모 입력 필드 추가 (FR-03-04, 200자 제한). 참석 응답 시 선택적으로 짧은 메모 작성 가능. ParticipantItem에 메모 truncate 표시. rsvpSchema에 note 필드 추가 (nullable, optional)
  - 관련 파일: `app/events/actions.ts`, `components/events/rsvp-form.tsx`, `components/events/participant-item.tsx`
  - 의존성: P3-03, P3-04

- [x] **P6-06** 참여자 제거 기능 구현
  - 상세: `removeParticipant` Server Action (주최자만, FR-03-05). 참여자 + 이벤트 주최자 조인 쿼리로 권한 검증. ParticipantItem에 주최자용 UserMinus 아이콘 제거 버튼. toast 피드백
  - 관련 파일: `app/events/actions.ts`, `components/events/participant-item.tsx`
  - 의존성: P3-04, P3-05

- [x] **P6-07** 최대 인원 제한 로직 구현
  - 상세: respondToEvent에서 참석 응답 시 max_participants 검증 (본인 기존 응답 제외 카운트). 최대 인원 초과 시 참석 응답 차단 + 에러 메시지. RsvpForm에 isFull prop으로 "참석 인원이 마감되었습니다" 경고 + 참석 버튼 비활성화. EventCard에 "마감" 텍스트 표시 (amber-600)
  - 관련 파일: `app/events/actions.ts`, `components/events/rsvp-form.tsx`, `components/events/event-card.tsx`
  - 의존성: P3-03, P2-04

- [x] **P6-08** 반응형 디자인 최적화 및 최종 마무리
  - 상세: (1) sm: breakpoint 기반 모바일 반응형 디자인 (grid, flex 레이아웃). (2) 홈페이지(`/`)에서 로그인된 사용자는 `/events`로 리다이렉트. (3) html lang="ko" 설정. (4) metadata 업데이트 (title: "모임 이벤트 관리", description 설정)
  - 관련 파일: `app/page.tsx`, `app/layout.tsx`, `app/events/page.tsx`
  - 의존성: 모든 Phase 완료

### 완료 기준 (Definition of Done)

- [x] 모든 사용자 액션에 Toast 피드백이 제공됨
- [x] 로딩 중 스켈레톤 UI가 표시되고, 데이터 없을 때 빈 상태 UI가 표시됨
- [x] 주최자가 이벤트를 삭제할 수 있고 확인 다이얼로그가 표시됨
- [x] 주최자가 이벤트 상태를 active/cancelled/completed로 변경 가능
- [x] 참여자가 메모를 작성하고 수정할 수 있음
- [x] 주최자가 참여자를 제거할 수 있음
- [x] 최대 인원 초과 시 참석 응답이 차단됨
- [x] 모바일 환경에서 반응형 레이아웃 적용됨
- [x] 로그인된 사용자가 홈페이지 접속 시 /events로 리다이렉트됨
